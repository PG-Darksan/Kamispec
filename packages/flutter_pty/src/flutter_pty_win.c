...
/* ── HisatorNotebook でのパッチ ──────────────────────────────────────
 *
 * 元の実装は executable を書いた後に arguments を丸ごと繋いでいた。
 * しかし arguments の先頭には Dart 側が既に executable を入れている
 * (lib/flutter_pty.dart:88 の `argv[0] = executable`)。 そのため
 * コマンドラインが `npm.cmd npm.cmd install -g pkg` となり、 npm は
 * 自分のパスを「コマンド名」 として受け取って Unknown command で落ちる。
 * 引数無しの `claude.cmd` でも `claude.cmd claude.cmd` になり、 CLI が
 * 自分のパスを指示文として受け取る。
 *
 * Unix 側 (flutter_pty_unix.c) は execvp(path, argv) で argv[0] が
 * プログラム名として消費されるので正しい。 これは Windows 限定の不具合。
 *
 * あわせて引用符も付ける。 元は一切付けていなかったので、
 * `C:\Program Files\nodejs\npm.cmd` のような空白入りのパスでトークンが
 * 割れ、 同じ階層に `Program.exe` があればそちらが起動してしまう。
 *
 * 先頭に空白を置かないこと。 CreateProcessW が ERROR_INVALID_PARAMETER で
 * 落ちる。
 * ────────────────────────────────────────────────────────────────── */
static LPWSTR build_command(char *executable, char **arguments)
{
    /* argv[0] が既に executable なので、 ここでは書き足さない。 */
    (void)executable;
