...
class PdfMemo {
  ...
  /// 添付画像のローカルパス (= ユーザー要望: メモに Ctrl+V で画像を貼り付け)。
  /// `attachments/` 以下に保存したファイルパス。null なら画像なし。
  /// テキストと併存可能 (画像 + 補足テキスト)。
  String? imagePath;
...
class MindMapNode {
  ...
  /// ピン留めされた PDF メモフォルダの名前リスト (= ユーザー要望
  ///   「PDFビューワーのフォルダーもピン留めできるように」)。ここに含まれる
  ///   フォルダはメモパネルの一覧上部に固定表示される。null/空なら
  ///   ピン留めフォルダなし。
  List<String>? pdfMemoPinnedFolders;
  ...
  MindMapNode copyWith({
    ...
    Object? pdfMemoPinnedFolders = _sentinel,
    ...
    return MindMapNode(
      ...
      pdfMemoPinnedFolders: pdfMemoPinnedFolders == _sentinel
          ? this.pdfMemoPinnedFolders
          : pdfMemoPinnedFolders as List<String>?,
  ...
  Map<String, dynamic> toJson() {
    return {
      ...
      if (pdfMemoPinnedFolders != null && pdfMemoPinnedFolders!.isNotEmpty)
        'pdfMemoPinnedFolders': pdfMemoPinnedFolders,
  ...
  factory MindMapNode.fromJson(Map<String, dynamic> json) {
    ...
    return MindMapNode(
      ...
      pdfMemoFolders: (json['pdfMemoFolders'] as List<dynamic>?)
          ?.map((e) => e as String)
          .toList(),
      pdfMemoPinnedFolders: (json['pdfMemoPinnedFolders'] as List<dynamic>?)
          ?.map((e) => e as String)
          .toList(),
